...
## [2026-08-29] merge | PR #11 (feat: add holdfold CLI and MCP server, extract shared verdict core) | pages touched: 0

PR #11 squash-merged to main (commit `b941a78`) after two review-fix rounds (see the two ingest entries above). Final state: `backend/core.py` (shared verdict engine), `backend/cli/` (`holdfold` CLI), `backend/mcp_server/` (`holdemfoldem-mcp` stdio server), plus the fixes for 9 total verified CodeRabbit findings across both rounds (AnalysisUnavailableError handling, ImportError on missing MCP path, fractional-qty P&L, None trade_raw guard, 4xx routing narrowed to 400/422, non-dict JSON body guard, mixed-side lot rejection, Cloud Run core.py packaging, pytest-asyncio dependency declaration). 34 pytest tests, 9 Playwright e2e tests, all green at merge time. Local branch `feat/cli-and-mcp` retained (not checked out to `main`) per the stay-on-branch-after-merge convention — no wiki page content changed by the merge itself beyond this log line.
