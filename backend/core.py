"""
Hold Em or Fold Em — verdict engine (transport-agnostic).

Extracted from main.py so the FastAPI app, the CLI, and the MCP server can all
call the same logic without importing FastAPI or triggering route
registration. main.py is now a thin HTTP adapter over compute_verdict().
...
class AnalysisUnavailableError(RuntimeError):
    """Raised when the upstream MCP Finance data pipeline fails."""
...
def validate_symbol(raw_symbol: str) -> str:
    ...
    Args:
        raw_symbol: User-supplied symbol, any case/whitespace.

    Returns:
        The uppercased, stripped symbol.

    Raises:
        ValueError: If empty or it doesn't match the ticker pattern.
    """
    ...
    if not _SYMBOL_RE.match(symbol):
        raise ValueError(
            f"Invalid symbol: '{symbol}'. Use 1-12 alphanumeric characters, "
            f"dots, or hyphens (e.g. AAPL, BTC-USD)."
        )
    return symbol


def validate_period(period: str) -> str:
    """Validate a lookback period against VALID_PERIODS.

    Raises:
        ValueError: If period is not one of VALID_PERIODS.
    """
...
async def compute_verdict(req: AnalyzeRequest, request_id: str | None = None) -> HoldFoldVerdict:
    """Run the full analyze pipeline and build a verdict.

    Transport-agnostic core of what was the POST /api/analyze route. Callers
    (FastAPI, CLI, MCP server) are responsible for translating ValueError /
    AnalysisUnavailableError into their own error surface.

    Args:
        req: A validated AnalyzeRequest.
        request_id: Optional caller-supplied correlation id; a fresh uuid4 is
            generated when omitted (e.g. non-HTTP callers).
    ...
    Raises:
        ...
        AnalysisUnavailableError: If the upstream MCP Finance pipeline fails.
    """
    request_id = request_id or str(uuid.uuid4())

    symbol = validate_symbol(req.symbol)
    period = validate_period(req.period)
    ...
    try:
        ...
        raise AnalysisUnavailableError(f"Analysis failed: {e}") from e
...
async def check_backend_health() -> dict:
    """Report whether the Firestore cache is reachable.

    Returns:
        {"status": "ok", "version": CORE_VERSION, "firestore": bool}
    """
    fs = _get_firestore()
    return {"status": "ok", "version": CORE_VERSION, "firestore": fs is not None}
